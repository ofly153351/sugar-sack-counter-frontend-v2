...
interface BoxRowProps {
  rowNumber: number;
  onDelete: () => void;
  onDataChange?: (data: BoxRowFormData) => void;
...
export default function BoxRow({
  ...
  useEffect(() => {
    if (aiResult && onDataChange) {
      ...
      console.log("🔍 [DEBUG] BoxRow - Extracted image paths:", {
        originalImagePath,
        annotatedImagePath,
  ...
  }, [aiResult, manualCount, onDataChange, rowNumber]);
  ...
  useEffect(() => {
    ...
    const savedDetectionType = localStorage.getItem(
      `boxRow_${rowNumber}_detectionType`
    );
    if (
  ...
  useEffect(() => {
    ...
    console.log(
      `🔄 BoxRow ${rowNumber}: Reset ALL data for fresh start`,
  ...
  useEffect(() => {
    const savedSessionId = localStorage.getItem(
      `boxRow_${rowNumber}_sessionId`
    );
    if (savedSessionId) {
      setSessionId(savedSessionId);
    } else {
      // Generate session ID in useEffect to avoid hydration mismatch
      const newSessionId = `box_session_${rowNumber}_${Math.random()
        .toString(36)
        .substr(2, 9)}`;
      setSessionId(newSessionId);
      localStorage.setItem(`boxRow_${rowNumber}_sessionId`, newSessionId);
    }
  }, [rowNumber]);
  ...
  const handleAutoDetectToggle = (enabled: boolean) => {
    ...
    localStorage.setItem(`boxRow_${rowNumber}_autoDetect`, enabled.toString());
  };

  const handleDetectionTypeChange = (type: "sack" | "box" | "both") => {
    setDetectionType(type);
    localStorage.setItem(`boxRow_${rowNumber}_detectionType`, type);
  };
  ...
  const handleAutoAIDetection = async (file: File) => {
    ...
    try {
      ...
      const result = await processImageWithAI(
        file,
        detectionType,
        true, // Always auto-save to MinIO
        sessionIdForAI,
        rowNumber,
  ...
  const handleSaveToMinIO = async () => {
    ...
    try {
      ...
      Swal.fire({
        ...
        html: `
          <div class="text-left">
            <p class="mb-2">บันทึกภาพสำเร็จ</p>
          </div>
        `,
        icon: "success",
        confirmButtonText: "ตกลง",
      ...
      Swal.fire({
        ...
        text: "ไม่สามารถบันทึกภาพ: " + (error.message || "เกิดข้อผิดพลาด"),
        icon: "error",
        confirmButtonText: "ตกลง",
      });
    } finally {
